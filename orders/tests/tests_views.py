...
class BednaScanViewTests(ViewsTestBase):
	...
	def test_scan_detail_shows_mark_zakaleno_action_with_permission(self):
		...
		self.b_eur_pr.save(update_fields=["stav_bedny"])
		permission = Permission.objects.get(codename="scan_mark_bedna_zakaleno")
		self.user.user_permissions.add(permission)
	...
	def test_scan_detail_hides_mark_zakaleno_action_with_admin_reklamace_permission(self):
		self.b_eur_pr.stav_bedny = StavBednyChoice.DO_ZPRACOVANI
		self.b_eur_pr.save(update_fields=["stav_bedny"])
		self.user.user_permissions.add(Permission.objects.get(codename="mark_bedna_zakaleno"))
	...
	def test_scan_detail_shows_mark_zakaleno_action_for_navezeno(self):
	...
	def test_sarze_scan_move_view_get_renders_form(self):
		...
		self.assertEqual(response.context["form"].initial["zacatek"], time(14, 35))
	...
	def test_scan_zakaleno_get_rejects_admin_reklamace_permission(self):
		self.b_eur_pr.stav_bedny = StavBednyChoice.DO_ZPRACOVANI
		self.b_eur_pr.save(update_fields=["stav_bedny"])
		self.user.user_permissions.add(Permission.objects.get(codename="mark_bedna_zakaleno"))
	...
	def test_scan_zakaleno_get_renders_confirmation_for_navezeno(self):
		self.b_eur_pr.stav_bedny = StavBednyChoice.NAVEZENO
		self.b_eur_pr.save(update_fields=["stav_bedny"])
		permission = Permission.objects.get(codename="scan_mark_bedna_zakaleno")
		self.user.user_permissions.add(permission)
...
class PracovistePrehledViewTests(TestCase):
	...
	def test_lists_only_active_non_loading_workplaces_and_multiple_open_batches(self):
		...
		self.assertContains(response, "Typ šarže")
		self.assertContains(response, "Železo")
		self.assertContains(response, "Prázdná")
